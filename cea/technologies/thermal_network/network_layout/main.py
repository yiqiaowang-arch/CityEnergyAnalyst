...
def network_layout(config, locator, output_name_network=""):
    ...
    path_output_substations_shp = locator.get_temporary_file("nodes_buildings.shp")
    ...
    path_total_demand = locator.get_total_demand()

    # Calculate points where the substations will be located
    calc_substation_location(input_buildings_shp, path_output_substations_shp, connected_buildings)

    # Claculate potential network
    calc_connectivity_network(path_default_arcgis_db, path_streets_shp, path_output_substations_shp,
    ...
    # calc_minimum_spanning_tree(path_potential_network, output_network_folder, path_output_substations_shp, output_edges,
    #                            output_nodes, weight_field, type_mat_default, pipe_diameter_default)
    calc_steiner_spanning_tree(path_potential_network, output_network_folder, path_output_substations_shp, output_edges,
                               output_nodes, weight_field, type_mat_default, pipe_diameter_default, type_network,
                               path_total_demand, create_plant, config.network_layout.allow_looped_networks)
